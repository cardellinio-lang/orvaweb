...
async function main() {
  // Clean old products
  const deleted = await prisma.product.deleteMany({ where: { category: { not: 'orva' } } });
  if (deleted.count > 0) console.log(`🧹 Deleted ${deleted.count} old product(s)`);

  // Upsert orva products
